...
async def _count_queries(client: EisClient, ru: list[str], models: list[str],
    ...
    async def one(q) -> dict:
        ...
        try:
            ...
            # за один запрос программа проходит ограниченное число страниц —
            # сказать об этом надо до прогона, а не замечанием после него
            cap = settings.eis_max_pages * settings.eis_page_size
            if row["contracts"] > cap:
                row["note"] = (f"столько за один запрос не взять: программа "
                               f"просмотрит первые {cap}. Сузьте период или "
...
def _remarks(res: RunResult) -> list[str]:
    ...
    # чем строка попала в отчёт — вопрос не менее важный, чем чего в ней нет:
    # оговорки отбора видны в колонке «Совпадение с запросом», и найти их в
    # файле можно только если сводка сказала, что искать
    doubt = sum(1 for r in rows if r.pos.match_doubt == "исполнение")
    unnamed = sum(1 for r in rows if r.pos.match_doubt == "бренд")
    if doubt:
        out.append(f"У {doubt} позиций модель названа только в перечне "
                   f"исполнений регистрационного удостоверения — какое из них "
                   f"поставлено, из контракта не видно. В файле это написано "
                   f"в колонке «Совпадение с запросом».")
    if unnamed:
        out.append(f"У {unnamed} позиций сошлась модель, но бренд в контракте "
                   f"не назван и производитель его не подтвердил — стоит "
                   f"просмотреть такие строки глазами.")
    return out
