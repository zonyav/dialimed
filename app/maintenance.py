...
AREAS: dict[str, dict] = {
    ...
    "ai": {
        "title": "Ответы ИИ по позициям",
        "what": "Что ИИ ответил про каждый текст позиции. Пока они здесь, "
                "повторный прогон по тем же позициям бесплатен и даёт тот же "
                "результат.",
        "why": "Стоит стереть, если хотите спросить ИИ заново — например, "
               "после смены модели.",
    },
}
...
def autocompact() -> dict:
    ...
    # Ответы ИИ не режем и по сроку: за них заплачено, и повторный прогон по
    # тем же позициям обязан дать тот же отчёт. Их стирают только вручную.
    before = size_mb()
