...
from app.timing import human

check("пока рано — молчим", _eta("контракты", 2, 100, 3.0), 0)
check("половина этапа контрактов", _eta("контракты", 50, 100, 100.0) > 0, True)
check("к концу прогона остаток меньше",
      _eta("вид", 9, 10, 100.0) < _eta("контракты", 50, 100, 100.0), True)
check("замедление увеличивает остаток",
      _eta("контракты", 50, 100, 200.0) > _eta("контракты", 50, 100, 100.0), True)
check("чужой этап не оценивается", _eta("старт", 0, 1, 100.0), 0)
