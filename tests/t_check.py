...
check("бесконечный поиск обрывается",
      _диалог(['{"поиск": "а%d"}' % i for i in range(12)],
              выдача=NameSearch(items=[{"noRu": "РЗН 1"}])).why,
      "не уложился в отведённые шаги")
check("повтор того же запроса обрывает диалог",
...
print("\n реестр пишет семейство, контракт — модель")
from app.enrich.rzn import shares_mark

_БИРЮСА = {"name": 'Камеры холодильные медицинские "Бирюса" по ТУ 28.25.13-020',
           "modelsDescription": "450S-R; 450K-R; 280S-R"}
check("обозначение стоит и там и там",
      shares_mark('Камера холодильная медицинская "Бирюса 450S-R"', _БИРЮСА), True)
check("наименование контракта целиком внутри реестрового",
      shares_mark("РЗН 2019/9387 Камеры холодильные медицинские", _БИРЮСА), True)
check("артикул ищется по перечню исполнений",
      shares_mark("Камера холодильная 450K-R", _БИРЮСА), True)
check("чужое изделие не сходится",
      shares_mark('Аппарат низкочастотной физиотерапии "Амплипульс-5ДС"',
                  {"name": "Электроды для низкочастотных электротерапевтических "
                           "процедур, одноразовые", "modelsDescription": ""}), False)
check("общих слов мало для совпадения", shares_mark("Аппарат", _БИРЮСА), False)

print("\n один завод, написанный по-разному")
from app.enrich.verify import same_company

check("аббревиатура и расшифровка",
      same_company('ОАО "ДЗМО"', "Досчатинский завод медицинского оборудования"), True)
check("приписанная страна — не спор",
      same_company("Сателек САС", "Сателек САС Франция"), True)
check("страна через запятую тоже",
      same_company("Кирхнер и Вильгельм ГмбХ, Германия", "Кирхнер и Вильгельм ГмбХ"), True)
check("разные заводы", same_company('ЗАО "ЗАВОД ЭМА"', 'ООО "Диксион"'), False)
check("похожее имя — ещё не тот же завод",
      same_company('ООО "Медтехника"', 'ООО "Медтехника плюс"'), False)
check("транслитерацию сводит человек, не программа",
      same_company("Диксион", "DIXION"), False)
...
class _Шлюз:
    ...
    def spent(self): return {"запросов к ИИ": 0, "токенов": 0}
...
def _пусто(n=1):
    return [_строка("Гистероскоп EndoGlance", trademark="EndoGlance")
            for _ in range(n)]
...
# строка из одних общих слов КТРУ модели ничего не даёт — и денег не стоит
_ГОЛАЯ = [_строка("Система электрохирургическая")] + _проверочные(3)
_РЕЗ = _прогон(_ГОЛАЯ, {"РЗН 2019/8462": _ЗАПИСЬ_ИИ},
               {question(r.pos.contract_text(), r.pos.specs_text): _ХОРОШО
                for r in _ГОЛАЯ})
check("про строку без зацепки не спрашиваем", _РЕЗ.stats.get("ИИ"), None)
...
check("имя важнее длины",
      search_words("Система ультразвуковая РуСкан 70П"), ["РуСкан"])
check("заглавная не в начале фразы — это имя",
      search_words("Камера холодильная Бирюса 450"), ["Бирюса"])
check("больше двух слов не спрашиваем",
      len(search_words("B.Well PRO-30 Comfort")), 2)
...
print("\n модель и бренд: где написано и насколько уверенно")
from app.query import article_keys, judge, judge_any, parse

_Q = parse("Ajax AJ15")
check("слово с цифрой — артикул, слово из букв — имя",
      ([t.raw for t in _Q.names], [t.raw for t in _Q.articles]),
      (["Ajax"], ["AJ15"]))
check("приставка через пробел клеится к числу",
      [t.raw for t in parse("AJ 15").articles], ["AJ15"])
check("приметный артикул стоит сам за себя", parse("Ajax AJ15").standalone, True)
check("артикул с цифры сам за себя не стоит", parse("рускан 70п").standalone, False)
check("«AJ 15» в тексте — тот же артикул",
      "aj15" in article_keys("Установка Ajax AJ 15 с принадлежностями"), True)

_ПОЗИЦИЯ = "Установка стоматологическая"
_ОДНА = "Установка стоматологическая, вариант исполнения: AJ15"
_ВСЕ = ("Установка стоматологическая, варианты исполнения: "
        "AJ11, AJ12, AJ15, AJ16, AJ18")
check("модель без бренда — это совпадение",
      judge("Ajax AJ15", _ПОЗИЦИЯ, _ОДНА).ok, True)
check("и оно помечено оговоркой",
      judge("Ajax AJ15", _ПОЗИЦИЯ, _ОДНА).doubt, "бренд")
check("бренд рядом с моделью снимает оговорку",
      judge("Ajax AJ15", _ПОЗИЦИЯ + " Товарный знак: Ajax", _ОДНА).doubt, "")
check("перечень исполнений РУ — сомнительное совпадение",
      judge("Ajax AJ15", _ПОЗИЦИЯ + " Товарный знак: AJAX", _ВСЕ).doubt,
      "исполнение")
check("чужое исполнение не подходит",
      judge("Ajax AJ18", _ПОЗИЦИЯ, _ОДНА).ok, False)
check("товарный знак называет другое исполнение — строка не наша",
      judge("Ajax AJ15", _ПОЗИЦИЯ + " Товарный знак: AJ11", _ВСЕ).ok, False)
check("а то же самое — наша",
      judge("Ajax AJ15", _ПОЗИЦИЯ + " Товарный знак: Ajax AJ15", _ВСЕ).doubt, "")
check("производитель подтверждает бренд",
      judge("Ajax AJ15", _ПОЗИЦИЯ, _ОДНА,
            "Guangzhou Ajax Medical Equipment Co., Ltd.").doubt, "")
check("реестр не подтверждает чужой бренд",
      judge("Olympus CV170", _ПОЗИЦИЯ, "Видеосистема CV-170",
            "Guangzhou Ajax Medical Equipment").doubt, "бренд")
check("артикул с цифры без имени не проходит",
      judge("рускан 70п", "Система ультразвуковая", "Аппарат ЭГД-70П").ok, False)
check("имя с артикулом с цифры проходит",
      judge("рускан 70п", "Система ультразвуковая РуСкан 70П").ok, True)
check("одного имени по-прежнему достаточно",
      judge("рускан", "Система ультразвуковая РуСкан 60").ok, True)
check("несколько строк запроса — это «или»",
      judge_any(["Olympus", "Ajax AJ15"], _ПОЗИЦИЯ + " Товарный знак: Ajax",
                _ОДНА).ok, True)
check("из двух совпадений берётся уверенное",
      judge_any(["Ajax AJ15", "Ajax"], _ПОЗИЦИЯ + " Товарный знак: Ajax",
                _ОДНА).doubt, "")
...
from app.pipeline import _earlier, _looks_like_service

for _имя, _ждём in (("Техническое обслуживание аппаратов «МАИА-01»", True),
                    ("Текущий ремонт:: Аппарат ингаляционной анестезии", True),
                    ("Калибровка СИ: Монитор анестезиологический", True),
                    ("Оказание услуг по техническому обслуживанию", True),
                    ("Наркозно-дыхательный аппарат МАИА-01", False),
                    ("Аппарат ингаляционной анестезии, передвижной", False)):
    check(f"«{_имя[:34]}» — {'работа' if _ждём else 'поставка'}",
          _looks_like_service(Position(name=_имя)), _ждём)


def _отбор_услуг(drop):
    poss = [Position(name="Наркозно-дыхательный аппарат МАИА-01"),
            Position(name="Техническое обслуживание аппаратов «МАИА-01»"),
            Position(name="Текущий ремонт:: Аппарат «МАИА-01»")]
    res = RunResult()
    rows = _collect_rows([(ContractMeta(reestr_number="1"), poss)], set(), res,
                         text=["МАИА-01"], drop_services=drop)
    return len(rows), res.stats.get("строк обслуживания и ремонта исключено", 0)


check("обслуживание и ремонт в отчёт не идут", _отбор_услуг(True), (1, 2))
check("но их можно оставить", _отбор_услуг(False), (3, 0))
check("коды ищем раньше периода отчёта",
      (_earlier("01.01.2025", "01.01.2026"), _earlier("01.01.2025", "01.01.2025")),
      (True, False))
